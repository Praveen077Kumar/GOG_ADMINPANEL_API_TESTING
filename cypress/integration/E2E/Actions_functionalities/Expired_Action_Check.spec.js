...
describe('Actions Page Functionality', function(){ 
    ...
    it('Checking the Action Select Page Title ',function(){
        ...
        actions.getFilterSelect().select('Expired')
       sharedObj.getSectionName().should('have.text',": Showing Expired actions")
    ...
    it('Checking Recently "Expired" Action and Viewing Action', function() {
       
        actions.getFilterSelect().select('Expired').then(()=>{   
        if((sharedObj.getTableRows()).should('be.visible')){
           sharedObj.getFirstRow().within(()=>{
               sharedObj.getButtonColumn().click()
            })
            sharedObj.getSectionNameButton().should('have.text','Expired')
