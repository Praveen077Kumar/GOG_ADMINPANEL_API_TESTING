...
describe('Create Product Gift Action With the API Functionality',()=>{
    ...
    it('Perform the Create Product Gift Transfer Action and then Fetch the Key and varify the status of Transfer ', () => {
        let responsedata
        cy.fixture('api/CriticalActions/DropUnpublishAction').then(myFixture => { 
        cy.request({
            method:"POST",
            ...
            if(responseMessage ==='"Drop has not yet been published"'){
                expect(response.status).deep.equal(400)
               throw new Error('Error! 400 ,Drop has not yet been published!!!')
            }
            else{
            ...
            cy.wait(5000).then(()=>{
                ...
                cy.request({
                    ...
                    cy.log("Done...Your Drop Has been UnPublished.")
                })
            })
            }
        }); 
